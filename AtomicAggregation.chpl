module AtomicAggregation {
    ...
    proc _flushBuffer(loc: int, ref bufferIdx, freeData) {
      ...
      on Locales[loc] {
        for (dstAddr, srcVal) in rBuffer.localIter(remBufferPtr, myBufferIdx) {
          dstAddr.deref().add(srcVal, memoryOrder.relaxed);
        }
        if freeData {
  ...
  record AtomicCopyDstAggregator {
    ...
    inline proc write(ref dst: AggregatedAtomic(elemType), const in srcVal: elemType) {
      // Get the locale of dst and the local address on that locale
      const loc = dst.locale.id;
      const dstAddr = getAddr(dst);
    ...
    proc _flushBuffer(loc: int, ref bufferIdx, freeData) {
      ...
      on Locales[loc] {
        for (dstAddr, srcVal) in rBuffer.localIter(remBufferPtr, myBufferIdx) {
          dstAddr.deref().write(srcVal, memoryOrder.relaxed);
        }
        if freeData {
          rBuffer.localFree(remBufferPtr);
        }
